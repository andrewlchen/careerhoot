...
def authenticate_user(request):
	...
	3. Authenticate user and log in using Django library. 

	4. Redirect to "sync" view
	'''

	attributes = create_attributes(request)
	save_attributes(request, attributes)
			...
			if grid_list[i]['positions']['_total'] > 0: # if there are more than 0 positions listed 
				while k < len(grid_list[i]['positions']['values']): # while k is less than the number of positions listed 
					if grid_list[i]['positions']['values'][k]['isCurrent'] == True: # if the position is currently active
						company_object = Company.objects.get_or_create(node_company=grid_list[i]['positions']['values'][k]['company'].get('name', 'No company specified'), node_title=grid_list[i]['positions']['values'][k].get('title', 'No title specified'))
...
def select_group(request, group_pk, current_user):
	...
	if group_pk == None: 
		...
		# current_user_grid = Grid.objects.all() # current_user.grid_set.all()
		...
		if selected_group.name == 'My own connections for: %s' % current_user.email: 
			...
			for u in selected_group_users: 
				...
				if agg_grid_sets: 
					agg_grid_sets = agg_grid_sets | u_grid_set
				else: 
					agg_grid_sets = u_grid_set

			current_user_grid = agg_grid_sets

			# then iterate through each of those grid items...
			for grid_item in agg_grid_sets: 

				connectors_for_grid_item = grid_item.connectors.all()

				'''
				Three conditions: 
				XXXXXXXXXXXXXXXXXXXXX (1) if the grid_item hasn't already been captured in the 
				accumulator list, 
				...
				if (grid_item.node_mid == current_user.username) or (current_user in connectors_for_grid_item): 
					current_user_grid = current_user_grid.exclude(node_mid=grid_item.node_mid)
				# else: 
					# for each connector in this grid item...
				for c in connectors_for_grid_item: 
					# ...if that connector is NOT connected in the 1st degree to the current_user by virtue of being in his core grid_set, then exclude from queryset. 
					if current_user_grid.filter(node_mid=c.username).exists(): 
...
def group(request, group_pk=None):
	...
	current_user_grid = select_group(request, group_pk, current_user) # , grid_list_range
	...
	for person in current_user_grid:
		...
		for company in company_set:
			if company not in current_user_companies: 
				current_user_companies.append(company)
	
	# sort current_user_grid on company axis
	current_user_companies = sorted(current_user_companies, key=lambda x: x.node_company)
...
def industry(request, sort_filter=None, view_filter='/introkick/industry/'):
	...
	# sort current_user_grid on industry axis
	current_user_grid = current_user_grid.order_by('node_industry')
...
def add(request):
	...
	typeahead_list = group_typeahead()
...
def request_access(request, requester=None):
	...
	# build e-mail contents: requester's name, e-mail, mid, group she is seeking admission to + its group_pk, and selected person's e-mails
		...
		# Send e-mail out
		subject = 'Approve access for %s - %s' % (requester_name, requester_email)
		body = 'Click this link (%s) to approve access for %s (%s, %s) to the group %s at primary key %s. If you don\'t know this person or do not wish to approve access, then simply ignore this e-mail.' % (embed_url, requester_name, requester_username, requester_email, requested_group_object, requested_group_pk)
...
def grant_access(request, group_pk=None, requester=None):
	...
	try: 
		...
		grant_access_notification = 'We\'re sorry, but %s %s has not requested admission into the group %s.' % (requester_user.first_name, requester_user.last_name, requested_group_object)
	else: 
		...
		if (requester_mid.invite_count > 0) and (requester_user not in Group.objects.get(id=group_pk).user_set.all()):
			...
			subject = 'Access has been approved for %s %s - %s' % (requester_user.first_name, requester_user.last_name, requester_user.email)
			body = 'Your access has been approved to the group %s at primary key %s.' % (requested_group_object, group_pk)
			from_address = 'IntroKick Notifications <archimedes@careerhoot.com>'
		...
		# show error notification to grantor 
			...
			grant_access_notification = 'We\'re sorry, but %s %s has either already been accepted into the group %s, or has not yet requested admission into the group.' % (requester_user.first_name, requester_user.last_name, requested_group_object)
...
def invite_to_group(request):
	...
	if invite_others_to_group.is_valid():
		...
		# if user is submitting the default email example address 
			...
			subject = 'Invitation to IntroKick, Group %s' % target_group.name
			body = '%s %s (%s) has invited you to join the group %s on IntroKick, a micro-network to make professional professional introductions easier! Click here (%s) to sign in with LinkedIn, join the group %s, and start kicking off intros!' % (user.first_name, user.last_name, user.email, target_group.name, sign_in_url, target_group.name)
			from_address = 'IntroKick Notifications <archimedes@careerhoot.com>'
			to_address = [cd['email']]
			message = send_email(subject, body, from_address, to_address)

			# render confirmation banner notification to inviter 
			request.session['user_notification'] = 'Thanks for sending a group invitation to %s!' % cd['email']
...
def create_grid_dictionaries(i, g, company, grid, current_user_grid):
	...
	if current_user_companies: 
		for company in current_user_companies:
			...
			for g in company_grid_members:
				grid = create_grid_dictionaries(i, g, company, grid, current_user_grid)
				i += 1
	else: 
		for g in current_user_grid: 
			g_companies = g.company_set.all()
