...
def company(request):
	...
	group = 'Not part of a group - add one below!'
	...
	if 'group' in request.POST:
		try: # see if that group exists 
			...
			group = Group.objects.create(group_name=request.POST['group'])


		# company page showing 'Not part of a group - add one below!' 
		# on first load, refresh. Empty being shown when submit button pressed
		# empty. Group name not being shown when legit one is submitted. 

		try: 
			current_user_profile.groups.get(group_name=group)
		except(KeyError, UserProfile.DoesNotExist):
			current_user_profile.groups.add(group)
			current_user_profile.save()
	else: 
		try: # see if that group exists 
			group_empty = Group.objects.get(group_name=group)
		except(KeyError, Group.DoesNotExist): # if not, create it 
			group_empty = Group.objects.create(group_name=group)
		current_user_profile.groups.add(group_empty)

	'''
	Need to add index to context below: 
	'current_group' : current_user_profile.groups.all() - does not support reverse indexing

	Create string with the group name, then assign to current_group var in below contexts. But how to create a string from the Group object? 
	'''
	...
	if 'email' in request.POST:
		if (request.POST['email'] == '') or (request.POST['email'] == 'Enter your email (no spam, ever)'):
			...
			return render_to_response('introkick/company.html', 
				...
				'email' : "Enter your email (no spam, ever)",
				'db_email' : request.user.get_profile().user.email,
				'current_group' : current_user_profile.groups.get(group_name=group),
