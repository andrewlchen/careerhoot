		...
		if selected_group.name == '%s %s\'s 1st degree connections' % (current_user.first_name, current_user.last_name): 
			...
			for u in selected_group_users: 
				...
				for grid_item in grid_set_for_u: 
					...
					if (grid_item not in current_user_grid) and (grid_item.node_mid != current_user.username) and (current_user not in connectors_for_grid_item): 
...
def create_grid_dictionaries(i, g, grid, current_user_grid_set, company, filter_lens):
	...
	if filter_lens == 'industry': 
		grid[i]['title'] = company.values_list('node_title', flat=True)
		grid[i]['company'] = company.values_list('node_company', flat=True)
...
def ajax(request): 
	...
	the results of a user clicking on a specific company or industry. 
	...
	try: 
		...
		for member in grid: 
			data += "<tr><td><a href=\"" + \
				member['public_url'] + \
				"\"><img alt=\"NODE FIRST LAST NAME\" id=\"node_profile_picture\" class=\"img-rounded\" src=\""

			if  member['picture_url'] == 'No picture given':
				data += "http://www.vdvl.nl/wp-content/uploads/2012/10/icon_no_photo_no_border_80x80.png"
			else: 
				data += member['picture_url']
			...
			data += "\" /></a></td><td><a href=\"" + \
				...
				"</li></ul></td><td><ul><li>" + \
				member['company'] + \
				"</li></ul></td><td>Connected through: "

			connectors = range(len(member['connectors']))

			for index in connectors:
				data += "<a href=\"" + \
					member['connector_urls'][index] + \
					"\">" + member['connectors'][index] + \
					"</a>"

				if index < len(member['connectors'])-1: 
					data += ", "
		...
		for each_person in expanded_industry: 
			...
			if each_person in current_user_grid: 
				...
				for company in each_person_companies: 
...
def home(request):
	...
	if current_user_companies: 
		for company in current_user_companies:
			# this gets all grid members who are part of that company 
			company_grid_members = company.grid.all()
			for g in company_grid_members:
				# this culls company_grid_members down to just those grid members who were assembled into current_user_grid from the function select_group()
				if g in current_user_grid: 
		...
		for g in current_user_grid: 
			...
			companies = g.company_set.all()
			filter_lens = 'industry'
			grid = create_grid_dictionaries(i, g, grid, current_user_grid_set, companies, filter_lens)
			i += 1
