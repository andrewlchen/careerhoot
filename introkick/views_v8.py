...
# current_user.groups.get(id=group_pk)
	...
	'''
	Need to make sure you only include grid objects for 2nd deg contacts where 
	1st deg contact shares same group as user. Currently, you are exposing the 
	user's whole grid. 
	'''
	...
	if ('group' in request.POST) and (request.POST['group'] != ''): # if user typed in group name, try to find it, else create
		...
		# company page showing 'email default group' on first load, refresh. 
		# Empty being shown when submit button pressed empty. Group name not 
		# being shown when legit one is submitted. 
	...
	else: # on initial load, load this
