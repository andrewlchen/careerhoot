...
def oauth_authenticated(request):
	...
	try:
		...
		try: 
			confirm_user = InviteMid.objects.get(mid=user.username)
			if get_request:
				target_group = Group.objects.get(id=get_request)
				user.groups.add(target_group)
				confirm_user.delete()
				request.session['group_member_form_notification'] = 'Congratulations on joining the group %s' % target_group
		except (InviteMid.DoesNotExist):
			if get_request:
				request.session['group_member_form_notification'] = 'We\'re sorry, but %s %s was not invited to the group %s' % (user.first_name, user.last_name, Group.objects.get(id=get_request))
		...
		if get_request: 
			user.groups.add(Group.objects.get(id=get_request))
...
def sync(request):
	...
	for i in grid_list_range:
		try: # see if that mid exists 
			...
			current_user_grid = []
			filtered_grid_objects = Grid.objects.filter(node_mid=mid)
		...
		if selected_group.name == 'My own connections for: %s' % current_user.email: 
			...
			for u in selected_group_users: 
				...
				for grid_item in grid_set_for_u: 
					...
					if (grid_item not in current_user_grid) and (current_user not in connectors_for_grid_item): 
...
def add(request):
	...
	if ('group' in request.POST) and (request.POST['group'] != ''): # if user typed in group name, try to find it, else create
		try: 
			...
			group_members = show_this_group.user_set.all()
			current_user.save()
			request.session['group_member_form_notification'] = 'You\'ve created the group: %s' % show_this_group
			return HttpResponseRedirect('/introkick/home/')
...
def request_access(request, requester=None):
	...
	if 'group_member' not in request.POST: # if user selected nothing, then re-render page with error message
		...
		invitee = InviteMid.objects.get_or_create(mid=requester_username)

		request.session['group_member_form_notification'] = 'Your e-mail request to join %s was sent!' % requested_group_name
...
def grant_access(request, group_pk=None, requester=None):
	...
	try: 
		...
		if requester_user not in Group.objects.get(id=group_pk).user_set.all():
...
def invite_to_group(request):
	...
	if target_email != request.user.email: 
		...
		try: 
			target_username = User.objects.get(email=target_email).username

			if not InviteMid.objects.filter(mid=target_username).exists():
				invitee = InviteMid.objects.create(mid=target_username)

				invite_to_group_notification = 'Thanks for sending a group invitation to %s!' % target_email
				...
				invite_to_group_notification = 'The user with the e-mail %s has already been invited to this group.' % target_email
		...
		request.session['group_member_form_notification'] = 'It appears you are inviting yourself to the group %s, which unfortunately isn\'t allowed.' % target_group.name
		return HttpResponseRedirect('/introkick/home')
...
def invite(request):
	...
	if target_email != request.user.email: 
		...
		request.session['group_member_form_notification'] = 'Thanks for sending an invitation to %s!' % target_email
		...
		request.session['group_member_form_notification'] = 'You entered your own e-mail address. Did you mean to invite yourself?'
...
def home(request): # , current_user_grid, grid_list_range, show_this_group, all_groups, sort_filter, group_pk=None
	...
	for g, gindex in zip(current_user_grid, grid_list_range):
		...
		for c in connectors_for_g_range:
